...
/// <summary>
/// Generates a Vega-Lite JSON spec from a DataTable.
/// Library: https://github.com/vega/vega
/// </summary>
public class VegaChartGenerator
{
    ...
    {
        _chartGenerators = new Dictionary<string, Func<DataTable, string?, VegaSpec>>(StringComparer.OrdinalIgnoreCase)
        {
    ...
    public class VegaSpec
    {
        ...
        public string Width { get; set; } = "container";
    ...
    {
        if (!_chartGenerators.TryGetValue(chartType, out var generator))
        {
            throw new ArgumentException($"Unsupported chart type: {chartType}");
        }

        var vegaSpec = generator(dataTable, title);
    ...
    private static List<Dictionary<string, object?>> ConvertDataTableToVegaData(DataTable dataTable)
    {
    ...
    private static string InferDataType(Type type)
    {
    ...
    private VegaSpec GenerateBarChart(DataTable data, string? title)
    {
        ...
        return new VegaSpec
        {
            ...
            Encoding = new VegaEncoding
            {
                ...
                Y = new VegaChannel
                {
                    Field = numericCol?.ColumnName ?? columns[Math.Min(1, columns.Count - 1)].ColumnName,
                    Type = InferDataType(numericCol?.DataType ?? columns[Math.Min(1, columns.Count - 1)].DataType),
                    Title = numericCol?.ColumnName ?? columns[Math.Min(1, columns.Count - 1)].ColumnName
                }
            }
        };
    }
    ...
    private VegaSpec GenerateLineChart(DataTable data, string? title)
    {
        var columns = data.Columns.Cast<DataColumn>().ToList();
        var xCol = columns[0];
        var yCol = columns[Math.Min(1, columns.Count - 1)];
    ...
    private VegaSpec GeneratePieChart(DataTable data, string? title)
    {
        ...
        var valueCol = columns[Math.Min(1, columns.Count - 1)];
